######################################### "Fleishman - Ver. 14  Exper Modified on 9.19.23"
############################  Copied from Posit Cloud
...
############################   1/9/2022 I'm "tieing off" this script to serve as a re-entry point.
...
for( Combo in 1:Cases) {
  ...
  dim(TableofConstantsII) <- c(43,73,4)
  ...
  for(reps in StartReps:MaxReps) { ## Apple
    ...
    LookUpCol <- 10*KurtEst + 13  ############### Beta
    
    LookUpCol <- round(LookUpCol)
    LookUpCol<- max(1,LookUpCol)
    LookUpCol <- min(73,LookUpCol)
	...
	if(TempSkew2 < 0) Adjust2 <- -1 * (.0285*TempSkew2^2 - .2332*abs(TempSkew2) + .0049)
      ...
      ##################2/25/2022 Why am I doing the 3 steps immed. below?
	##################  9/18/23 As can be seen, I'm "commenting" out this crap especially since there is no
	##################          TempSkew2 equivalentn.
      #TempSkew <- 100*TempSkew
      #TempSkew <- ceiling(TempSkew)
      #TempSkew <- TempSkew/100
	...
	LookUpRow <- round(LookUpRow)
      ...
      LookUpRow <- min(43,LookUpRow)
        ...
        #if(TempSkew > SkewLim.Unreal) {   			### Apple
        #Skew.Flag <- 1
        #RealReps <- RealReps - 1
        #Duds <- Duds + 1
        #break
        ...
        #LookUpCol <- 10*KurtAbove1 + 13  ##  Temp Exp on 4/9/2023
        LookUpCol <- 13
	...
	LookUpRow2 <- round(LookUpRow2)  
      ...
      LookUpRow2 <- round(LookUpRow2)
        ...
        #LookUpCol2 <- 10*KurtAbove2 + 13  ## Temp Exp on 4/9/2023
        LookUpCol2 <- 13
	...
	## 9/7/2023 Exp below
       ...
       if(SkewNeg2.Flag == 0) NullData2[,1] <-  Con2.2[1,1] + Con2.2[2,1]*RandSamp2[,1] + Con2.2[3,1]*RandSamp2[,1]^2 + Con2.2[4,1]*RandSamp2[,1]^3 	
        ...
        if(TwoSample == 0) {
          ...
          if( Method == 3) {	
            ...
            #ScoresBoot[boots] <- ( median(NullData)-median(NullData2) + TwoSamp.Adj ) 
            #/ (mean(sd(NullData.mod),sd(NullData2.mod) ) ) ### Epsilon
            
            #ScoresBoot[boots] <-  (   (median(NullData) + TwoSamp.Adj.1)- (median(NullData2) + TwoSamp.Adj.2)    )/ 
            #(mean(IQR1,IQR2) )  ### Epsilon
            
            #ScoresBoot[boots] <-  (   (median(NullData) )- (median(NullData2) )    )/ 
            #(mean(IQR1,IQR2) )  ### Epsilon
            ...
            #(mean(sd(NullData.mod),sd(NullData2.mod) ) ) 
            
            #################  Tiger
            
            #ScoresBoot[boots] <- ( median(NullData) - median(NullData2) ) /
              #mean( sd(NullData[,1]), sd(NullData2[,1] )   ) 

		Alpha <- .1 
            
           	#1 <-  +.3*(1-exp(-Alpha* abs(TempSkew) ))
 		#Adjust1 <-  -.3*(1-exp(-1* TempSkew ))
		#y = 0.0285x2 - 0.2332x + 0.0049
      ...
      if(TwoSample == 0) {
       ...
       if( Method == 2) {  #  Rho
        ...
        if( Method == 3) {
          ...
          #Score <- ( median(NData) - median(NData2) + TwoSamp.Adj ) / 
          #(   mean(sd(NData), sd(NData2) )   ) #################  Epsilon
          
          #Score <- ( median(NData) - median(NData2) + TwoSamp.Adj ) / 
          #( mean(IQR1, IQR2) )    #################  Epsilon
          
          #Score <-  (   (median(NData) + TwoSamp.Adj.1)- (median(NData2) + TwoSamp.Adj.2 )    ) / 
          #(mean(IQR1,IQR2) ) 
          
          #Score <-  (   (median(NData) )- (median(NData2)  )    ) / 
          #(mean(IQR1,IQR2) ) 
          
          #Score <- ( median(NData) - median(NData2)  ) / 
          #(   mean(sd(NData), sd(NData2) )   )
          
          #Score <-  ( median(NData - NData2)  ) / 
          #(   mean(sd(NData), sd(NData2) )   )
		...
		Adjust2 <- Adjust2 * Adjust.Fudge
          ...
          Score <- ( (median(NData) + Adjust1 ) / sd(NData) )
          - ( (median(NData2) + Adjust2) / sd(NData2)   ) 

 		##### Exp. on 9/19/23  All of the commenting out in the 3 groups below.
		#if(  (TempSkew >= 0) & (TempSkew2 < 0) ) {
		#Ignore.Flag <- 1
		#NoCount <- NoCount + 1
		#}
      ...
      if(Skew.Unreal ==1) {
        #write("Skew.Unreal")
        #write(Skew.Unreal)
        CritVal1 <- quantile(ScoresBoot,.005)
        CritVal2 <- quantile(ScoresBoot,.995)
